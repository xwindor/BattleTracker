...
const fs = require("fs");
const { createSessionStore, createRoomCode, hasPersistableContent } = require("./server/session-store");
const {
...
const store = createSessionStore({ fs, path, dir: DATA_DIR });
 ...
 *
 * There are **three** server-side write sites, not one: `session:update-state`,
 * `session:append-log`, and the in-place `ownerName` strip in the disconnect
 * handler. They all come through here so a fourth site added later cannot
 * quietly skip persistence (spec, Proposed approach part 1).
 */
 ...
 * any claimable participant they own, persist (write site 3 of 3) and
 * rebroadcast, so a returning player can re-claim cleanly.
 *
...
io.on("connection", (socket) => {
   ...
   * so a handler added later cannot reopen the `session:command` hole by
   * forgetting to opt in - it would have to opt out on purpose.
   *
  ...
  socket.on("player:join", ({ room, playerName } = {}, cb) => {
    ...
    if (typeof cb === "function") {
      ...
      cb({ ok: true, state: session.state, log: session.log, playerName, gmConnected: isGmConnected(room) });
    }
  });
  ...
  socket.on("session:update-state", ({ room, state }) => {
    ...
    const session = getOrCreateSession(room);
    session.state = state;
    touchSession(room); // write site 1 of 3
  ...
  socket.on("session:append-log", ({ room, entry }) => {
    ...
    touchSession(room); // write site 2 of 3
  ...
  socket.on("disconnect", () => {
    ...
    // Write site 3 of 3, and the one the spec calls out as easiest to miss:
    // `releasePlayerClaims` mutates `session.state.participants` in place
    // without going through `session:update-state`, so without its own
    // `touchSession` call an ownership release would never reach disk.
    releasePlayerClaims(room, playerName);
  });
});
