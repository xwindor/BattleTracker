...
3. Changing the DP input updates the initiative base immediately (DP + INT).
4. Clicking HOT/COLD/AR mode buttons sets dices to 4/3/1 and updates initiative.
5. PHYS LOCKED badge appears when mode is COLD or HOT.
...
3. When OS crosses 20 → amber banner: "⚠ IC Alert — [Decker name] OS: 21".
4. When OS crosses 40 → red modal: "☠ Convergence! GOD attacks [Decker name]".
...
3. On Spawn: an ICParticipant appears in the tracker with `baseIni = rating × 2`, dices = 2 (Patrol) or 4 (others).
...
**Acceptance criteria:**
1. Clicking "+Mark" on a target card: dropdown selects decker → mark count increments (max 3).
2. Dots rendered: e.g., `●●○` for 2 marks.
3. "×" button next to each decker's dot row removes 1 mark.
4. Cannot place a 4th mark (button disabled at 3).
...
**Acceptance criteria:**
1. "Hack on the Fly" button opens a dice roll prompt (reusing DiceRollerComponent) then the OS prompt (+2 per mark on success, per RAW).
2. "Brute Force" button does the same with opposed test hints and OS cost (marks × 4).
3. "Direct Connection" checkbox: sets accessMethod = 'direct-connection'; 1 mark on host, 0 OS, confirmation log entry.
...
**Goal:** "Switch Two Matrix Attributes" (Free Action) lets the decker swap any two of A/S/D/F. If DP changes, initiative base updates immediately.
